...
export default function ResultsPage() {
  ...
  useEffect(() => {
    const fetchClaim = async () => {
      ...
      // Retry briefly to handle read-after-write timing right after submission.
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        if (supabaseError) {
          lastErr = supabaseError;
          console.error(`Claim fetch attempt ${attempt + 1} failed:`, supabaseError);
        } else if (data) {
      ...
      if (lastErr) {
        setError(`Database error: ${lastErr.message}`);
      } else {
        setError("This claim could not be found. It may have been deleted, the link is incorrect, or your account doesn't have permission to view it.");
      }
      setLoading(false);
    };
