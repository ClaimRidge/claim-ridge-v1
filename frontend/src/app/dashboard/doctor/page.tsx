...
export default function DoctorDashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // The doctor sees only their own claims (scoped by user_id).
      ...
      // Pre-auths the doctor submitted.
      try {
        ...
        if (session) {
          ...
          if (res.ok) setPreAuths((await res.json()) as PreAuthRow[]);
        }
      } catch {
  ...
  return (
    ...
    // with the insurer and provider dashboards.
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 flex flex-col">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex gap-2 flex-wrap w-full sm:w-auto">
          <Link href="/dashboard/doctor/pre-auth/new" className="flex-1 sm:flex-none">
            <Button variant="outline" className="gap-2 w-full"><ShieldCheck className="h-4 w-4" /> New Pre-Auth</Button>
          </Link>
          <Link href="/dashboard/doctor/claims/new" className="flex-1 sm:flex-none">
            <Button className="gap-2 w-full"><FilePlus className="h-4 w-4" /> New Claim</Button>
          </Link>
        </div>
      </div>
          ...
          {FILTER_TABS.map((tab) => {
            ...
            return (
              ...
              >
                {tab.label}
                {count > 0 && (
                  <span className={`ml-1.5 text-xs ${activeFilter === tab.key ? "text-white/80" : "text-[#9ca3af]"}`}>
                    ({count})
                  </span>
                )}
              </button>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-[#f3f4f6]">
                  {filteredClaims.map((claim) => (
                    <tr
                      key={claim.id}
                      onClick={() => router.push(`/dashboard/doctor/claims/${claim.id}/results`)}
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="flex flex-col items-start gap-1.5">
                          <Link
                            href={`/dashboard/doctor/claims/${claim.id}/results`}
      ...
      <div className="order-1">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          ...
          <Link
            href="/dashboard/doctor/pre-auth"
        ...
        {preAuthLoading ? (
          ...
          <div className="bg-white border border-[#e5e7eb] rounded-xl shadow-sm p-12 text-center">
            ...
            <p className="text-[#9ca3af] text-sm mb-4">Pre-auths you submit will show up here.</p>
            ...
            <div className="bg-white border border-[#e5e7eb] rounded-xl shadow-sm p-5 mb-6">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs text-[#6b7280]">
                  Approval rate{" "}
                  <strong className="text-[#16a34a]">{paDecided > 0 ? `${paApprovalRate}%` : "--"}</strong>
                </span>
              </div>
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-[#f3f4f6]">
                    {recentPreAuths.map((p) => (
                      <tr
                        key={p.id}
                        onClick={() => router.push(`/dashboard/doctor/pre-auth/${p.id}`)}
                        className="hover:bg-[#f9fafb] transition-colors cursor-pointer"
                      >
